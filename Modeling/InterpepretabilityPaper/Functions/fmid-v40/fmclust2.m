function [FM,Part,mu] = fmclust2(Dat,FM,out)
% FMCLUST build a MIMO static or dynamic fuzzy model from data.
%     Uses product-space fuzzy clustering (Fuzzy c-means).
%
...
MFTYPE = 3;     % type of membership functions
% MFTYPE is the membership function type
%			MFTYPE = 1 (trapezoidal)
%			MFTYPE = 2 (exponencial)	(default)
%			MFTYPE = 3 (sigmoidal)
...
for k = out                 % for given outputs
   ...
   for b = 1 : length(U)
      ...
      dat = [dat;[ur y1]];          % data to cluster (supervised)
