function plotcons_ARA(FM,opt,fig,options)
...
%   if nargin < 4, options{i} = diag(ones(nant,1)); end;
   ...
   for k = 1 : npar
      if sum(options{i}(:,k)) > 0
         figure(fig+k-1+outputoffset); clf; set(gcf,'numbertitle','off',...
             'name',vars{i}{k},'Units','centimeter',...
             'Position',[15 1 35 25.3],'PaperSize',[25 12]);
      end   
      jj = 0;
      antepos = 1;
      for j = 1 : nant
         if options{i}(j,k)
            ...
            %subplot(sp(npar/2,1),sp(npar,2)+2,jj);
            switch npar
               case 1, subplot(1,1,1);
               case {2,3}, subplot(npar,1,jj);
               otherwise, subplot(ceil(npar/4),4,jj);
            end;      
            if ~isempty(opt)
               errorbar(v,th,s,opt);
            else
...
%             set(gca,'XLim',[FM.zmin{i}(antepos) FM.zmax{i}(antepos)]);
            ...
            if strcmp(vars{i}{k},'offset')
               ylabel([vars{i}{k}],'FontSize',FontSize,'FontWeight','bold');
            else
               ylabel(['param at ' vars{i}{k}],'FontSize',FontSize,'FontWeight','bold');
            end   
            xlabel(avars{i}{j},'FontSize',FontSize,'FontWeight','bold');
            grid on
         end
      end
      drawnow;
