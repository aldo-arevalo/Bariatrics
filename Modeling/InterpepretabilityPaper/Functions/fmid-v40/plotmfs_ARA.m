...
if isnumeric(mfs) && ~isempty(mfs)
   ...
   for q=1:size(mfs,1)
       hold on
       dom2 = sort(normalize_matrix(dom));
       dom3 = sort(dom);
       plot(dom3,mgrade(dom2,mfs(q,:)),opt{q,1},'LineWidth',2,'DisplayName',c_display{q,1});
   end
   hold off
...
elseif iscell(mfs)
   ...
   for i = 1 : n
      ...
      plotmfs_ARA(mfs{i},opt,dom(:,i),features{i},c_display);
   end;
end;
