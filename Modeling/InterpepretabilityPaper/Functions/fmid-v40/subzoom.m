function out = subzoom(varargin)
%SUBZOOM   Zoom in and out on a 2-D plot.
%   SUBZOOM with no arguments toggles the zoom state.
%   SUBZOOM(FACTOR) zooms the current axis by FACTOR.
%       Note that this does not affect the zoom state.
%   SUBZOOM ON turns zoom on for the current figure.  
%   SUBZOOM OFF turns zoom off in the current figure.
%   SUBZOOM OUT returns the plot to its initial (full) zoom.
%   SUBZOOM XON or ZOOM YON turns zoom on for the x or y axis only.
%   SUBZOOM RESET clears the zoom out point.
%
...
%   
%   SUBZOOM(FIG,OPTION) applies the zoom command to the figure specified
%   by FIG. OPTION can be any of the above arguments.

%   SUBZOOM FILL scales a plot such that it is as big as possible
...
%
%   SUBZOOM XON zooms x-axis only
%   SUBZOOM YON zooms y-axis only
...
if strcmp(zoomCommand,'off'),
   ...
   %
   state = getuprop(fig,'ZOOMFigureStateSub');
   if ~isempty(state),
      uirestore(state);
      clruprop(fig,'ZOOMFigureStateSub');
   end
   return
end % if
...
elseif strcmp(zoomCommand,'on'),
   ...
   if isempty(state),
      state = uisuspend(fig);
      setuprop(fig,'ZOOMFigureStateSub',state);
   end
   set(fig,'windowbuttondownfcn','subzoom down', ...
...
elseif strcmp(zoomCommand,'xon'),
   subzoom(fig,'on') % Set up userprop
   set(fig,'windowbuttondownfcn','subzoom xdown', ...
      'windowbuttonupfcn','ones;', ...
      'windowbuttonmotionfcn','','buttondownfcn','',...
      'interruptible','on');
   set(ax,'interruptible','on')
...
elseif strcmp(zoomCommand,'yon'),
   subzoom(fig,'on') % Set up userprop
   set(fig,'windowbuttondownfcn','subzoom ydown', ...
      'windowbuttonupfcn','ones;', ...
      'windowbuttonmotionfcn','','buttondownfcn','',...
      'interruptible','on');
   set(ax,'interruptible','on')
...
list = subzoom(fig,'getconnect'); % Circular list of connected axes.
if zoomx,
   %if a(1)==a(2), return, end % Short circuit if zoom is moot.
   %set(ax,'xlim',a(1:2))
   set(findobj(gcf,'Type','axes'),'xlim',a(1:2))
   h = list(1);
   while h ~= ax,
      set(h,'xlim',a(1:2))
