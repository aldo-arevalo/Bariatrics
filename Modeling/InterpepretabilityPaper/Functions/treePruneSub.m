function [min_leaves, OptimalTree] = treePruneSub(trainingData,Options,MaxNumSplits)
...
min_leaves = round(leafs(err == min(err)));

OptimalTree = fitctree(...
    predictors, ...
    response, ...
    'SplitCriterion', 'gdi', ...
    'Surrogate', 'on', ...
    ...
    'AlgorithmForCategorical', 'Exact', ...
    'CategoricalPredictors', isCategoricalPredictor, ...
    'MinLeafSize',min_leaves, ...
    'MaxNumSplits', MaxNumSplits);
view(OptimalTree,'mode','graph')

% [~,~,~,bestlevel] = cvLoss(OptimalTree,...
%     'SubTrees','All','TreeSize','min', 'KFold', Options.k);
% view(classificationTree,'Mode','Graph','Prune',bestlevel)
% 
% classificationTree = prune(OptimalTree,'Level',bestlevel); 
% view(OptimalTree,'Mode','Graph')
